...
- Verdict จาก rubric ใน `SKILL.md`: Critical unsafe-to-ship → `block`; Critical must-fix → `request-changes`; Suggestion ถูกต้อง/security/สัญญา → `request-changes`; สไตล์อย่างเดียว → `approve-with-nits`; Nice-to-have → `approve-with-nits`; ไม่ชัวร์ → prefer `request-changes`  
